...
<div id="member_right" class="member_right b_record_box">
    <ul class="b_record_title">
        <ul class="b_record_title">
            <li class="{{ $type==0 ? 'b_record_this':'' }}"><a href="{{ url('user/buy',['type'=>0]) }}">进行中<span class="b_record_block">{{ $beforoLottery }}</span></a></li>
            <li class="{{ $type==1 ? 'b_record_this':'' }}"><a href="{{ url('user/buy',['type'=>1]) }}">即将揭晓<span class="b_record_block1">{{ $startLottery }}</span></a></li>
            <li class="{{ $type==2 ? 'b_record_this':'' }}"><a href="{{ url('user/buy',['type'=>2]) }}">已揭晓<span class="b_record_block2">{{ $overLottery }}</span></a></li>
        </ul>
                    ...
                    @foreach ($list as $val)
                        <tr>
                           ...
                           <td id="div_0"><div class="b_goods_name">
                                   <span><a href="{{ url('/product',['id'=>$val->object->id]) }}">(第{{  $val->object->periods }}期){{ $val->goods->title }}</a></span>
                                   <b class="b_all_require">价值:￥{{ $val->goods->money }}</b><div class="bgSilver" style="margin-right:0px"><span class="bgRed" style="width:{{round($val->object->participate_person/$val->object->total_person*100,2)}}%;"></span></div>
                                   <div style="overflow: hidden;"><span class="orange fl" style="width:33%;height: 36px;line-height: 16px;"><em style="display: block;">{{$val->object->participate_person}}</em>已参与</span><span class="gray6 fl" style="width:33%;height: 36px;line-height: 16px;text-align: center;"><em style="display: block;">{{$val->object->total_person}}</em>总需人次</span><span class="blue fr" style="width:33%;height: 36px;line-height: 16px;text-align: right;"><em style="display: block;">{{$val->object->total_person - $val->object->participate_person}}</em>剩余</span></div>
                               </div></td>
                           <td class="b_color"><span>进行中</span><span class="ongoing_append"><a onclick="addtoCart({{$val->g_id}},1)" href="javascript:void(0);">追加</a></span></td>
                           <td class="times0">{{ count(json_decode($val->buyno)) }}人次</td>
            ...
            <table id="soonTable">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td id="div_0"><div class="b_goods_name">
                                <span><a href="{{ url('/product',['id'=>$val->object->id]) }}">(第{{  $val->object->periods }}期){{ $val->goods->title }}</a></span>
                                <b class="b_all_require">价值:￥{{ $val->goods->money }}</b><div class="bgSilver2"><span class="bgRed2"></span></div>
                            </div></td>
                        <td class="b_soonTable">正在揭晓中</td><td class="times0">{{ count(json_decode($val->buyno)) }}人次</td>
                        <td><a href="javascript:;" onclick="yunNum('<?php echo url('/product',['id'=>$val->object->id]);?>',<?php echo $val->id;?>)"  class="viewAll2" >查看所有块乐码</a></td>
                    </tr>
                    @endforeach
                </tbody>
            ...
            <table id="ongoingTable">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td id="div_0"><div class="b_goods_name">
                                <span><a href="{{ url('/product',['id'=>$val->object->id]) }}">(第{{  $val->object->periods }}期){{ $val->goods->title }}</a></span>
                                <b class="b_all_require">获得者:{{ $val->user->nickname }}</b><b class="b_all_require">幸运码：{{ $val->object->lottery_code }}</b>
                                <b class="b_all_require">本期总需:{{ $val->object->total_person }}人次</b><b class="b_all_require">揭晓时间：{{ date('Y-m-d H:i:s',(int)($val->object->lottery_time/1000)) }}</b>
                            </div></td>
                        <td class="b_color"><span>已揭晓</span><span class="ongoing_append"><a onclick="addtoCart({{$val->g_id}},1)" href="javascript:void(0);">继续众筹</a></span></td>
                        <td class="times0">{{ count(json_decode($val->buyno)) }}人次</td>
                        <td><a href="javascript:;" onclick="yunNum('<?php echo url('/product',['id'=>$val->object->id]);?>',<?php echo $val->id;?>)" class="viewAll2">查看所有块乐码</a></td>
                    </tr>
                    @endforeach
