...
@section('my_css')
    ...
    <div class="w_con" id="goods_during">
        ...
        <div class="w_during_left">
            ...
            <div class="w_during_graphic">
                ...
                <div class="w_during_wen" style="position:relative;">
                    <h1 id="publishTitle">(第{{$data['goods_will']['periods']}}
                        期){{$data['goods_will']['belongs_to_goods']['title']}}</h1>
                    <div class="w_add_doing" id="publishWinCode">
                        <p class="w_add_doing_noleft">揭晓结果<b>幸运码</b></p>
                        <div class="w_addBg_other"><p class="w_results">{{$data['goods_will']['lottery_code']}}</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="w_during_winner" style="position:relative;">
                ...
                <div class="w_winner_left2">
                    <div class="w_winner_figure"><a href="/him/{{$fetchuser[0]['usr_id']}}"><img id="publishWinUserIcon"
                                                      src="{{$fetchuser[0]['user_photo']}}"/></a></div>
                    <dl class="w_winner_wen">
                        <dd class="w_red" style="text-overflow: ellipsis;white-space: nowrap;overflow: hidden;">恭喜<span
                                    id="publishWinUser"><a href="/him/{{$fetchuser[0]['usr_id']}}">{{$fetchuser[0]['nickname']}}</a></span> 获得了本期商品
                        </dd>
                        <dd>本期购买：<i id="publishWinBuyTimes">{{count($data['buynolucky'])}}</i>人次 <a id="lookYgCode" href="javascript:void(0)"
                                                                       class="w_other_ren" onclick="">点击查看</a></dd>
                        <dd id="publishWinTime">揭晓时间：{{$fetchuser[0]['kaijiang_time']}}</dd>
                        <dd id="publishBuyTime">购买时间：{{$fetchuser[0]['bid_time']}}</dd>
                    </dl>
                </div>
                @endif
                <!--揭晓后状态 end-->
                    ...
                    @if($usr_id>0)
                        @if(!empty($data['userbuywithid']))
                            <div class="w_winner_lucky">
                                @if(!empty($data['buyno']))
                                    <i>您本期一块购@if(!empty($fetchuser[0]['usr_id'])&&$fetchuser[0]['usr_id']==$usr_id)幸运码@else块乐码@endif为：</i>
            ...
            <div class="w_big_text_box">
                ...
                <ul class="w_calculate">
                    ...
                    <ul class="w_calculate_in">
                        <li class="w_details_first"><em id="publishWinCodeCalculate">{{$data['goods_will']['lottery_code']}}</em><span>本期幸运号码</span>
                        </li>
                        <li class="w_details_second " onmouseenter="Times100Enter()" onmouseleave="Times100Leave()"><em
                                    id="ygRecord100">{{$data['total_time']}}</em><span>100个时间求和</span>
                            <div class="w_two_con">
                                <i></i>
                                <p class="w_two_text">奖品的最后一个号码分配完毕，公示该分配时间点前本站全部奖品的<strong>最后100个参与时间</strong>，并求和。</p>
                            </div>
                        </li>
                        <li class="w_details_fourth"><em id="publishTotalPrice">{{$data['goods_will']['total_person']}}</em><span>该奖品总需人次</span></li>
        ...
        <div class="w_during_right">
            ...
            <div class="w_latest_right1" style="position:relative;">
                <div class="w_rightImg"><a href='/product/{{$data['goods']['id']}}' class="w_goods_img" id="cartImg" /><img src="{{$data['goods']['belongs_to_goods']['thumb']}}"/></a>
                </div>
            ...
            <div class="w_buy"><a id="buybuybuy" href="javascript:void(0)"  data-gid={{$data['goods']['belongs_to_goods']['id']}} onclick="javascript:void(0)">立即抢购</a></div>
        </div>
        <div class="w_clear"></div>
        <!--选项卡-->
        @include('foreground.product_tab2')
        <!--人气商品-->
    ...
    <div class="luckymen_numberbox" style="display:none">
        @if(!empty($data['buynolucky']))
            @foreach($data['buynolucky'] as $key=>$val)
                <span class="l_number">{{$val}}</span>
            @endforeach
        @endif
    </div>
