            ...
            <div id="div_evaldata" class="ng-data-inner">
                ...
                <div class="ng-data-detail">
                    <div class="ng-data-step">
                        <p class="title">截止该商品最后购买时间【{{$data['lasttime']}}】网站所有商品的最后100条购买时间(时、分、秒、毫秒)记录</p>
                        <div class="step">
                            <ul class="step-inner clearfix">
                                ...
                                <li class="s-t">=</li>
                                <li class="s-t">(</li>
                                <li class="s-r2"><p>{{$data['total_time']}}</p><span>以下100条时间取值之和</span></li>
                                <li id="li_mod" class="s-t mod"><i>%</i><span class="txt">(取余)</span></li>
                                <li class="s-r3"><p>{{$data['goods_will']['total_person']}}</p><span>总需参与人次</span></li>
                                <li class="s-t">)</li>
                                <li class="s-t">+</li>
                                <li class="s-r4"><p>100000001</p><span>固定数值</span></li>
                                <li class="s-t">=</li>
                                <li class="s-r5"><p>{{$data['goods_will']['lottery_code']}}</p><span>最终计算结果</span></li>
                            </ul>
                    ...
                    <div class="ng-table-wrapper">
                        <div id="div_nginner" class="ng-table-inner">
                            <ul class="ng-table-ul clearfix">
                                @foreach($data['buyrecord'] as $kc=>$vc)
                                    @if($kc<100)
                                        <li>
                                            <span class="time"><b>{{$vc['lottery_time1']}}</b>{{$vc['lottery_time2']}}</span>
                                            <span class="code">{{$vc['lottery_time3']}}</span>
                                    <span class="name"><a target="_blank">@if(isset($vc['nickname'])){{$vc['nickname']}}@else{{config('global.default_nickname')}}@endif</a></span>
                                            <span class="num">{{$vc['buycount']}}人次</span>
                                    <span class="pro"><a href="/product/{{$vc['o_id']}}"
                                                         target="_blank">(第{{$vc['g_periods']}}
                                            期){{$vc['g_name']}}</a></span>
                                        </li>
                                    @endif
                                @endforeach
                ...
                <ul class="ng-table-ul ng-tc-ul">
                    @foreach($data['buyrecord'] as $k=>$v)
                        @if($k>99)
                            <li>
                                <span class="time"><b>{{$v['lottery_time1']}}</b>{{$v['lottery_time2']}}</span>
                                <span class="code"></span><span class="name"><a href=""
                                                                                target="_blank">@if(isset($vc['nickname'])){{$vc['nickname']}}@else{{config('global.default_nickname')}}@endif</a></span>
                                <span class="num">1人次</span>
                        <span class="pro"><a href="/product/{{$data['goods_will']['id']}}" target="_blank">(第{{$data['goods_will']['periods']}}
                                期){{$data['goods_will']['belongs_to_goods']['title']}}</a></span>
                            </li>
                        @endif
                    @endforeach
...
<input type="hidden" value="{{config('global.base_url').config('global.ajax_path.product_buyrecord')}}"
       id="recordpath"/>
