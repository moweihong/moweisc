...
@section('my_css')
    ...
    <div class="w_con">
        ...
        <div class="w_goods_nav">
            <input type="hidden" value="{{$data['cateid']}}" id="from_cateid"/>
            <h2>全部商品<span>（共<a href="/category" class="goodscount">{{$data['count']}}</a>件商品）</span></h2>
            <div class="w_choose">
                <dl class="w_choose_list clearfix">
                    @if(!empty($category['category']))
                    @foreach($category['category'] as $key=>$val)
                        <dd><a href="javascript:void(0);" data-id="{{$val['cateid']}}" class="w_selected"><span
                                        class="w_icon w_icon{{$key+1}}"></span><b>{{$val['name']}}</b></a></dd>
                    @endforeach
                    @endif
                </dl>
...
@section('my_js')
    <script type="text/javascript" src="{{ $url_prefix }}js/paginate.js"></script>
    ...
    <script type="text/javascript">
        ...
        $(window).load(function () {
            ...
            var catid = $('#catid').val();
            var url = "{{config('global.base_url')}}" + "category/postdata";
            if(par == "yes"){  //添加页面标签，判断 首次带链接参数 ?par=yes 进入“new” 分类
                var order = 'new';
                $(".w_new dd a").eq(3).addClass("w_announced").parents("dd").siblings("dd").find("a").removeClass("w_announced");
            }
            else{   //默认“default” 分类
                var order = 'default';
            }
        ...
        function flushPdt(page) {
            var catid = $('#catid').val();
            var url = "{{config('global.base_url')}}" + "category/postdata?page=" + page;
            var order = $('#order').val();
