...
<div class="header header_fixed">
    <div class="header1">
        <div class="header1in">
            <ul class="headerul1">
                ...
                <li class="cctv-icon" style="margin-left:20px;"><img style='margin:-8px 0 0 -8px' src="{{$url_prefix}}/img/cctv.png"></li>
            </ul>
            <ul class="headerul2">
                <?php if(session('user.id') > 0){  ?>
                <li style="margin-top:6px"><span class="h-ruwelcome" style="font-size: 14px">欢迎您，</span><a href="/user" style="color:#fb0603; display: inline">{{session('user.nickname')}}</a> <a href="/destory_login" style=" display: inline">[ <span>退出</span> ]</a></li>
                <li><a class='fg'>|</a></li>
                ...
                <li>
                 ...
                 <!-- <li><a class='fg'>|</a></li>
               <li class="cctv-icon"><img style='margin:-3px -3px 0 5px' src="{{$url_prefix}}/img/kf.png"></li>
                <li>
    ...
    <div class="header2">
        <a href="/index" class="header_logo"><img src="{{ $url_prefix }}img/logo.png"></a>
        <div class="search_header2">
            <s></s>
            <input type="text" placeholder="搜索您需要的商品" maxlength="30" value="" id="searchVal"/>
            <a href="javascript:actionSearch();" class="btnHSearch"><img src="{{$url_prefix}}/img/search.png"></a>
                       <ul class="hot-query">
...
<div class="yNavIndexOut yNavIndexOut_fixed yNavIndexOut_fixed_index">
    <div class="yNavIndex">
        <div class="pullDown">
            ...
            <ul class="pullDownList">
                @foreach($category['category'] as $key=>$val)
                    <li class="pullnav-li">
                        <a href="/category/{{$val['cateid']}}" class="dir_nav_a"><i class="dir_navico dir_navico{{$key+1}}"></i><em
                                    class="dir_navtit">{{$val['name']}}</em><span></span></a>
                        <div class="item-sub item-sub{{$key+1}} clearfix">
                            <ul>
                            	@if(isset($val['brand']))
                                @foreach($val['brand'] as $key2=>$val2)
                                    <li>
                                        <a href="/product/{{$val2->id}}" title="{{$val2->title2}}"alt="{{$val2->title2}}">
                                            <div class="item-simg"><img onerror="javascript:this.src='{{$url_prefix}}img/nodata/product-loading.png';" src="{{$val2->thumb}}" alt="{{$val2->title2}}" width="100"  height="100"/></div>
                                            <b class="item-stit">{{$val2->title2}}</b>
                                        </a>
                                    </li>
                                @endforeach
                                @endif
        ...
        <ul class="yMenuIndex">
            ...
            <div class="_mycart">
                <em class="cartEm"></em><span><a href="/mycart">我的购物车</a></span><i class="cartI" id="cartI"> {{ $total_count }}</i>
            </div>
        </ul>
    </div>
</div>
...
<div class="join-shop-car">
    <div class="car-item01"><img src="{{$url_prefix}}img/login_success.png"/>成功加入购物车</div>
