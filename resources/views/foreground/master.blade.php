...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="{{ $url_prefix }}css/comm.css?v={{config('global.version')}}"/>
    <link rel="stylesheet" type="text/css" href="{{ $url_prefix }}css/footer_header.css?v={{config('global.version')}}"/>
    <link rel="stylesheet" type="text/css" href="{{ $url_prefix }}css/goods.css?v={{config('global.version')}}"/>
    <link rel="stylesheet" type="text/css" href="{{ $url_prefix }}css/index.css?v={{config('global.version')}}"/>
    <script type="text/javascript" src="{{ $url_prefix }}js/jquery190.js"></script>
    <script type="text/javascript" src="{{ $url_prefix }}js/jquery.lazyload.min.js"></script>
    @if(isset($is_mobile))
    	<script>
			 var is_mobile = {{$is_mobile}};
			 var invite_code_m = {{$invite_code_m}};
             //手机型号访问pc端直接访问触屏手机版
                 ...
                 if (sUserAgent.indexOf(mobileAgents[i]) > -1) {
                     if(is_mobile == 1){
                        window.location.href = window.location.href+'index_m'
                    	 ...
                    	 var is_freeday = {{isset($is_freeday) ? $is_freeday : 0}};
	...
	<script type="text/javascript" src="{{ $url_prefix }}js/index.js?v={{config('global.version')}}"></script>
	<script type="text/javascript" src="{{ $url_prefix }}js/user_history.js?v={{config('global.version')}}"></script>
	<script type="text/javascript">
	var editurl=Array();
	...
	editurl['webupswf']="{{ asset('foreground/js/webupload/Uploader.swf') }}";
	</script>
	{{--全局js--}}
    {{--公共脚本--}}
    <script type="text/javascript" src="{{ $url_prefix }}js/common.js?v={{config('global.version')}}"></script>
    <script type="text/javascript" src="{{ $url_prefix }}js/layer/layer.js?v={{config('global.version')}}"></script>
    <script type="text/javascript" src="{{ $url_prefix }}js/my_cart.js?v={{config('global.version')}}"></script>
...
<body>
    ...
    {{--模板头文件--}}
    @include('foreground.header')
    {{--模板导航--}}
    @include('foreground.navigate')
    {{--模板内容--}}
    <div class="content">
        @yield('content')
    </div>
    {{--模板底部--}}
    @include('foreground.footer')

    {{--滑动条模块--}}
    @include('foreground.sidebar')
