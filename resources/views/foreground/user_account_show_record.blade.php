        ...
        <!-- E 我的晒单模块标题 -->
           ...
           <table id="sdTable" style="display:none">
								<tbody>
									...
									<tr>
									  <td><a href="javascript:void(0)"><img src="{{ $row->sd_thumbs }}" alt=""></a></td>
									  <td id="div_0"><div class="sd_goods_name">
									     <span><a href="javascript:void(0)">{{ str_limit($row->sd_title,22) }}</a></span>
										 <span>{{ date('Y-m-d H:i:s',$row->sd_time) }}</span><b class="sd_all_require">{{ str_limit($row->sd_content,30) }}</b>
									  </div></td>
									  <td class="sd_color">
									  	@if($row->is_show==1)
									  	<span class="sd_pass">审核通过</span>
									  	@elseif($row->is_show==0)
									  	<span class="sd_during">审核中</span>
									  	@else
									  	<span class="sd_pass">拒绝通过</span>
									  	<span class="sd_reward" style='color: red;'>{{ $row->refused_cause }}</span>
									  	@endif
									  </td>
									  ...
									  <td class="sd_color">
									  	@if($row->is_show==1)
									  	<span class="sd_reward">{{ $row->kl_bean }}</span>
									  	@else
                                      ...
                                      <td>
                                          ...
                                          <a id="wanshanAddress" href="javascript:void(0)" onclick="editShow({{$row->id}})">重新编辑</a>
                                          @else
                                          <a href="javascript:void(0)" onclick="showDetail({{$row->id}})" class="viewAll2">查看详情</a>
                                          @endif
                                      </td>
									</tr>
									@endforeach
           ...
           <table id="wsdTable" >
								<tbody>
									...
									@foreach($noshow as $v)
									<tr>
									  <td><a href="javascript:gotoGoods(327,725)"><img src="{{ $v->thumb }}" alt=""></a></td>
									  <td id="div_0">
									  	<div class="wsd_goods_name">
										     <span><a href="javascript:gotoGoods(327,725)">(第{{ $v->g_periods }}期) {{ $v->g_name }}</a></span>
											 <b class="wsd_all_require">价值：{{ $v->money }}</b>
											 <b class="wsd_all_require">幸运块乐码：{{ $v->fetchno }}</b>
											 <b class="wsd_all_require">揭晓时间:{{date('Y-m-d H:i:s',(int)($v->kaijiang_time/1000))}}</b>
									     </div>
									  </td>
									  <td class="sd_color"><span class="wsd_score">{{ $v->code }}</span></td>
									  <td><a href="javascript:;" class="wsd_yjsd" style="color:red" data-sd_gid="{{ $v->g_id }}" data-bid="{{ $v->id }}" data-sd_periods="{{ $v->g_periods }}" data-oid="{{ $v->o_id }}">一键晒单</a></td>
									</tr>
									@endforeach
								</tbody>
							</table>
...
<script type="text/javascript" src="{{ $url_prefix }}js/jquery.rotate.min.js"></script>
<script>
