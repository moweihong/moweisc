    ...
    <div class="c_main_box">
        ...
        <div class="sidebar_r clrfix fr">
            ...
            <div class="g-QR-code g-sid-title">
                ...
                <div class="clrfix">
                    <span class="fl"><a href="javascript:void(0);" >
                            <img style="width: 70px; height: 70px;" src="{{ $url_prefix }}img/wei.jpg"></a></span>
                    <p class="fl gray6">
                        扫一扫<br>
                        了解更多信息
                    </p>
            ...
            <div class="g-QR-code g-sid-title">
                ...
                <div class="clrfix">
                    <span class="fl"><a href="javascript:void(0);">
                            <img style="width: 75px; height: 75px;" src="{{ $url_prefix }}img/weixin.jpg"></a></span>
                    <p class="fl gray6">
                        扫一扫<br>
                        享受更多微信专享服务
                    </p>
       ...
       <!-- <ul class="c_main_income" style="margin-bottom:15px">
            <li id="c_money_type"  >
                ...
                <span id="accountUsableMoney">￥{{ $user->money}}</span>
            </li>
            ...
            <li class="c_income">
                ...
                <span style="color:#f57403;" id="redCount">{{ $user->kl_bean }}</span>
            </li>
            <div class="c_right_line"></div>
            <li class="c_income">
                <div class="c_money_type">红包(个)</div>
                <span style="color:#f57403;" id="JSDshopCard">{{ $redtotal }}</span>
            </li>
            <div class="c_right_line"></div>
            <li>
        ...
        <ul class="c_main_income">
            <li id="c_money_type"  >
                ...
                <span id="accountUsableMoney">{{ $orderSure }}</span>
            </li>
            <div class="c_right_line"></div>
            <li class="c_income">
                <div class="c_money_type">待发货<i></i></div>
                <span style="color:#f57403;" id="redCount">{{ $orderSend }}</span>
            </li>
            <div class="c_right_line"></div>
            <li class="c_income">
                <div class="c_money_type">待收货</div>
                <span style="color:#f57403;" id="JSDshopCard">{{ $orderGet }}</span>
            </li>
            <div class="c_right_line"></div>
            <li>
        ...
        <div class="account_show">
        	<ul class="item01"><li>账号余额：<span>{{ $user->money}}</span>元</li><li><input type="button" value="立即充值" onclick="window.location.href='/user/recharge_now'"/></li></ul>
        	<ul class="item02"><li>块乐豆：<span>{{ $user->kl_bean }}</span>个&nbsp;<img src="{{ $url_prefix }}img/question_ioc.png"/></li><li><input type="button" value="赚块乐豆" onclick="window.location.href='/user/score'" /></li></ul>
        	<ul class="item03"><li>红包：<span>{{ $redtotal }}</span>个</li><li><input type="button" value="立即一块购" onclick="window.location.href='/category'"/></li></ul>
        	<ul class="item04"><li><div><p>待支付</p><p class="red">0</p></div></li><li><div><p>已获得待确认</p><p class="red">{{ $orderSure }}</p></div></li><li><div><p>待发货</p><p class="red">{{ $orderSend }}</p></div></li><li><div><p>待收货</p><p class="red">{{ $orderGet }}</p></div></li></ul>
        	<div class="kld_tip">
        		<p>100块乐豆=1块钱,</p>
        ...
        <div class="g-acquired-goods g-common-control clrfix">
            ...
            @if($obtainOrder->count()>0)
            <ul>
                @foreach($obtainOrder as $val)
                <li>
                    <span class="fl"><a href="{{url('product',['id'=>$val->o_id])}}">
                            <img alt="" src="{{ $val->goods->thumb }}"></a></span>
                    <dl class="fl">
                        <dt><a class="wztxt" href="{{url('product',['id'=>$val->o_id])}}" class="gray3" title="{{ $val->goods->title }}">恭喜您获得<span>(第{{ $val->object->periods }}期){{ $val->goods->title }}</span></a></dt>
                        <dd>价值：￥{{ $val->goods->money }}</dd>
                        <dd>幸运块乐码：{{ $val->fetchno }}</dd>
                    </dl>
                    @if($val->status==4)
                    <a href="javascript::void(0);" data-id='{{$val->id}}' class="z-perfect-btn confirmGood">确认收货</a>
                	@endif
                	@if($val->status==2)
                	<a href="{{url('user/address',['orderid'=>$val->id])}}" class="z-perfect-btn">完善地址</a>
               		@endif
                </li>
                @endforeach
            </ul>
                ...
                <div class="commodity-list clrfix">
                 @if($yunOrder->count()>0)
                    <div id="div_UserBuyList" style="position: absolute; width: 1638px; left: 0px;">
                        @foreach($yunOrder as $val)
                        <div class="productsCon">
                            <div class="proList">
                                <ul>
                                    <li class="list-pic">
                                        <a  href="{{url('product',['id'=>$val->o_id])}}"><img src="{{ $val->goods->thumb }}"></a>
                                    </li>
                                    <li class="list-name"><a href="{{url('product',['id'=>$val->o_id])}}">(第{{ $val->object->periods }}期){{ $val->goods->title }}</a></li>
...
<script>
    ...
    ShareUrl = 'http://{{$_SERVER['HTTP_HOST']}}/freeday?code={{session('user.id')}}';
    ...
    function levelUp(){
        $.post("{{url('/user/levelup')}}",{'_token':"{{csrf_token()}}"},function(data){
            if(data==null){
               layer.msg('服务端错误');
            }
            if (data.status == 1){
