...
@section('content')
    <style>
		...
		.share-item .inner-item03{width: 253px;height: 55px;background: url({{ $url_prefix }}img/share_ioc01.png) no-repeat;font-size: 14px;line-height: 20px;color: #999999;padding: 20px 25px;}
    ...
    <!-- banner   start -->
    <input type="hidden" value="{{session('isout')}}" id="isout"/>
    <div id="banner_tabs" class="flexslider">
    	<ul class="slides">
        @foreach( $data['slide']  as $key=>$val)
            @if(!empty($val['img']))
            <li>
                <a href="{{$val['link']}}" @if($val->show_type == 1) target="_blank" @endif class="item" style="background-image:url({{$val['img']}})"></a>
				
		</li>
                {{--<a href="{{$val['link']}}" class="item" style="background-image:url({{$val['img']}})"></a>--}}
            @endif
        @endforeach
       </ul>
...
<input type="hidden" id="sys_cur_time" value="{{date('Y/m/d H:i:s')}}"/>
    ...
    @section('my_js')
    <script type="text/javascript" src="{{ $url_prefix }}js/jquery.SuperSlide.2.1.1.js"></script>
	...
	$('#banner_tabs .flex-next').click(function() {
        ...
        <script>
            ...
            function getlatest() { //最新揭晓 填充数据版
                var url = "{{config('global.base_url').config('global.ajax_path.index_latest')}}";
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: function (data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            strbody += '<li id="li'+data[i]['id']+'" class="current">' +
                                    ...
                                    'src="{{$url_prefix}}img/nodata/product-loading.png"' +
         ...
         /*   function getlatest_zhen() { //最新揭晓
            ...
            function getsoon() {
                //获取热门推荐商品
                var url = "{{config('global.base_url').config('global.ajax_path.index_soon')}}";
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: function (data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            strbody += '<div class="g-hotL-list">' +
                                    ...
                                    'src="{{$url_prefix}}img/nodata/product-loading2.png" data-original="' + data[i]['belongs_to_goods']['thumb'] + '"' +
            ...
            function getall() {
                //获取全部商品
                var url = "{{config('global.base_url').config('global.ajax_path.index_all')}}";
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: function (data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            strbody += '<ul>' + '<div class="soon-list-con"><div class="soon-list">' +
                                    ...
                                    'src="{{$url_prefix}}img/nodata/product-loading2.png"' +
                                    'data-original="' + data[i].belongs_to_goods.thumb + '"' +
                                    '>' +
                                    '</a>' +
                                    '</li>' +
            ...
            function getnew() {//获取最新商品
                var url = "{{config('global.base_url').config('global.ajax_path.index_new')}}";
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: function (data) {
            ...
            function getshow() {//获取晒单分享
                var url = "{{config('global.base_url').config('global.ajax_path.index_show')}}";
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: function (data) {
                            ...
                            if (i == 0) {
								strbody+='<div class="share-item">'+
										    '<a href="/sharedetail/'+data[i]['id']+'" target="_blank" title="' + data[i]['sd_title'] + '">' +
												'<div class="inner-item01"><img src="{{ $url_prefix }}img/nodata/product-loading3.png" class="lazy5" data-original="' + data[i]['sd_thumbs'] + '" /></div>'+
											'</a>'+
											'<a href="/him/'+data[i]['usr_id']+'" target="_blank" >' +
												'<div class="inner-item02"><img class="lazy5" src="{{$url_prefix}}img/def.jpg" data-original="'+data[i]['user_photo']+'" /><span class="user-nickname">'+data[i]['nickname']+'</span><span class="timestr">'+data[i]['sd_time']+'</span></div>'+
											'</a>'+
											'<a href="/sharedetail/'+data[i]['id']+'" target="_blank" title="' + data[i]['sd_title'] + '">' +
								 ...
								 strbody+='<div class="share-item01">'+
												'<a href="/sharedetail/'+data[i]['id']+'" target="_blank" title="' + data[i]['sd_title'] + '">' +
													'<div class="inner-item01"><img src="{{ $url_prefix }}img/nodata/product-loading2.png" class="lazy5" data-original="' + data[i]['sd_thumbs'] + '" /></div>'+
												'</a>'+
												'<a href="/him/'+data[i]['usr_id']+'" target="_blank" >' +
													'<div class="inner-item02"><img class="lazy5" src="{{ $url_prefix }}img/def.jpg" data-original="'+data[i]['user_photo']+'"/>'+
												'</a>'+
												'<a href="/sharedetail/'+data[i]['id']+'" target="_blank" title="' + data[i]['sd_title'] + '">' +
													'<div>'+
            ...
            function getfooter() { //获取底部数据填充
                var url = "{{config('global.base_url').config('global.ajax_path.index_footer')}}";
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: function (data) {
            ...
            function getarticles() {     //滚动公告栏
                var url = "{{config('global.base_url').config('global.ajax_path.index_articles')}}";
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: function (data) {
            ...
            function getnavigate() {    //全部商品分类
                var url = "{{config('global.base_url').config('global.ajax_path.index_navigata')}}";
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: function (data) {
            ...
            function getisbuy() {
                var url = "{{config('global.base_url').config('global.ajax_path.index_isbuy')}}";
                $.ajax({
                    type: 'GET',
                    url: url,
                    success: function (data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            if(!data[i]['userinfo']['user_photo']){
                            	data[i]['userinfo']['user_photo'] = {{$url_prefix}}img/nodata/tx-loading.png;
                            }
                            strbody += '<li>' +
