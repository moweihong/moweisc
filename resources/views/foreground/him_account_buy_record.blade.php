    ...
    <div id="member_right" class="member_right b_record_box">
	   ...
	   <div class="receive_confirm_new" id="error_content">
                ...
                <div class="not_enough_content">
                    ...
                    <p class="not_enough_money">客观，拨打客服电话400-6626-985<br>或重新确认号码试试吧</p>
                ...
                <table id="winTable">
                    <tbody>
                        <tr class="b_part_title">
                            ...
                            <th class="b_th3">夺宝状态</th>
                            <th class="b_th4">参与人次</th>
                            <th class="b_th6">夺宝号码</th>
                        </tr>
                        @foreach ($list as $val)
                            <tr>
                                ...
                                <td id="div_0"><div class="b_goods_name">
                                        ...
                                        <b class="b_all_require">总需：{{ $val->object->total_person }}人次</b>
                                        @if($val->fetchno>0)
                                        <b class="b_all_require">获得者：{{ $val->user->nickname }}</b>
                                        <b class="b_all_require">幸运码：{{ $val->fetchno }}</b>
                                        <b class="b_all_require">揭晓时间：{{ date('Y-m-d H:i:s',floor($val->kaijiang_time/1000))}}.{{substr($val->kaijiang_time,-3) }}</b>
                                        <b class="b_all_require">夺宝时间：{{ date('Y-m-d H:i:s',floor($val->bid_time/1000))}}.{{substr($val->bid_time,-3) }}</b>
                                        @endif
                                    </div></td>
                                <td class="b_color">
                                ...
                                <td>
                                    <?php 
                                    $buyno=json_decode($val->buyno);
                                  	$num=count($buyno);
                                   
                                    ?>
                                    <div class="b_clound_code">
                                    	<?php if($num>7){for($i=0;$i<5;$i++){?>
                                    	<i><?php echo $buyno[$i]?></i>
                                    	<?php } ?>
                                    	
		                                <div class="w-msgbox m-detail-codesDetail" id="b_cloud_window" style="margin-left:-347px;left:50%;top:50%;margin-top:-100px;display:none;">
											<a data-pro="close" href="javascript:void(0);" class="w-msgbox-close" id="w-msgbox-close"></a>	
											<div class="w-msgbox-hd" data-pro="header">
										    
										  </div>
											<div class="w-msgbox-bd" data-pro="entry">
												<div class="m-detail-codesDetail-bd">
													<div class="m-detail-codesDetail-wrap"><dl class="m-detail-codesDetail-list f-clear">
														<?php for($j=0;$j<$num;$j++){?>
														<dd><?php echo $buyno[$j];?></dd>
														<?php }?>
														</dl></div>
												</div>
											</div>	
										</div>	
                                    	<a href="javascript:void(0)" class="b_periods_num">查看更多></a>	
