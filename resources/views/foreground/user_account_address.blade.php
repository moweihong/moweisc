...
@section('content_right')
    <div id="member_right" class="member_right">
        ...
        <div class="my-address">
            <div class="c_address_top"><span></span>已保存{{count($address)}}条地址</div>
            <!--列表 start-->
            ...
            @foreach($address as $row)
                <div class="c_address_detail c_new_address_detail">
                    ...
                    <div class="c_address_operation">
                        ...
                        <?php }?>
                        <div class="c_address_all_operation updateAddress" data-id='{{$row->id}}' 
                        	 data-province='{{$row->province}}' data-city='{{$row->city}}' data-country='{{$row->country}}'
                        	 data-area='{{$row->area}}' data-receiver='{{$row->receiver}}' data-mobile='{{$row->mobile}}'
                        data-qqnum='{{$row->notice}}'>
                            <i   title="编辑" class="c_set_edit " style="cursor: pointer"></i>
                        </div >
                        <div  >
                        	 <i  data-id='{{$row->id}}' title="删除" class="c_set_esc deleteAddress" style="cursor: pointer"></i>
                        </div>
                    </div>
                    <div class="c_user_address"><p class="c_user_message">收 货 人 ：{{$row->receiver}}</p>
						  <p class="">收货号码：{{$row->mobile}}</p>
                    	  <p title="">收货地区：{{$row->province}}   {{$row->city}}  {{$row->country}}</p>
                    	  <p title="">收货地址：{{$row->area}}</p>
                    	  <p title="">备注：{{$row->notice}}</p>
                    </div>
                    <span></span>
                    @if($orderid != null)
                    <div class="c_packet_name" title="选择为发货地址">
                        <a href="javascript::void(0);" onclick="saveOrderAddr({{$row->id}},{{$orderid}})">使用</a>
                    </div> 
                    @endif
                </div>
             @endforeach  
                ...
                <div class="c_form_con">
                 ...
                 <div class="c_form_con">
                    ...
                    <div class="c_form_right">
                        ...
                        <span style="color:#555;font-size:14px;margin-left:20px;">（例：充值qq的号码为123456）</span>
                    </div>
                    <div class="c_clear"></div>
                </div>
...
@endsection
@section('myt_js')
	<script type="text/javascript" src="{{ $url_prefix }}js/area.js"></script>
