	  ...
	  <div class="p-reg-main" style='margin-top:0.08rem'>
         <div class="mui-input-group" >
            <div class="mui-input-row" >
               <label>收货人</label>
               <input id="name" type="text" value="{{ $name }}" maxlength="20" placeholder="输入姓名" onkeyup="value=value.replace(/[^a-zA-Z\u4E00-\u9FA5]/g,'');">
            </div>
			<div class="mui-input-row">
               <label>手机号码</label>
               <input id="phone" type="text" value="{{ $phone }}" maxlength="13" placeholder="输入手机号" onkeyup="value=value.replace(/[^0-9]/g,'').replace(/\s/g,'');">
            </div>
<!--			<div class="mui-input-row">
               <label>固定号码</label>
			...
			<div class="mui-input-row">
               <label>地址</label>
               <input id="showCityPicker3" type="text" value="{{ $address }}"  placeholder="XX省XX市XX县/区" maxlength="20">
            </div>

			<div class="mui-input-row">
               <label>详细地址</label>
               <input id="addrDetail" type="text" value="{{ $area }}" placeholder="输入详细地址" maxlength="35">
            </div>
            <div class="mui-input-row">
               <label>备注</label>
               <input id="notice" type="text" value="{{ $notice }}" placeholder="备注" maxlength="35">
            </div>
   ...
   <script>
       (function($) {
        ...
        $(".mui-btn").click(function(){
            ...
            $.post("/user_m/saveaddress", {'notice':notice,'id':{{$id}} ,'name': name,'phone': phone,'address': address,'addrDetail': addrDetail,'action':'update','_token':"{{csrf_token()}}"}, function(data){
                     if (data.status == 1) {
                        myalert(data.msg);
