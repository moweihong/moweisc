...
<div class="well">
    <form method="post" action="/backend/shop/dealUpShop">
	    ...
	    <tr><td>
	            商品标题：<textarea name="title" style="margin: 0px; width: 639px; height: 103px;" id='shoptitle'>{!! $shoplist->title !!}</textarea>
	    </td></tr>
	     <tr><td>       	    
    	...
    	<tr><td>
    	商品总价：<input style="width: 36px" size="6" type="text" name='money' value="{{ $shoplist->money }}">&nbsp元<br />
    	</td></tr>
    	...
    	<img src="{{ $shoplist->thumb }}" style="border:1px solid #eee; padding:1px; width:50px; height:50px;" id="thumpshow">
		 ...
		 <?php if(!empty($shoplist->picarr)){foreach($shoplist->picarr as $v){?>
                    	<tr>
			<td >           
			  <img src="{{ $v }}" style="border:1px solid #eee; padding:1px; width:300px; height:300px;" id="thumpshow">
            </td>           
		</tr> 
                    <?php }}?>            
		 <tr><td>&nbsp;</td></tr>
	...
	<br/>
	<p>PC推广链接：<a target="_blank" href="http://www.ts1kg.com/prod/{{$shoplist->id}}">http://www.ts1kg.com/prod/{{$shoplist->id}}</a></p>
	<p>H5推广链接：<a target="_blank" href="http://m.ts1kg.com/prod_m/{{$shoplist->id}}">http://m.ts1kg.com/prod_m/{{$shoplist->id}}</a></p>
</div>
