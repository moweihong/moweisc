      ...
      <tbody>
      	@foreach ($shoplist as $row)
            <tr>
            	...
            	<td>
					{!! $row->title !!}
					<p class='fbt'>副标题：{!! $row->title2 !!}<BR>供应商：<font color='#f60'>{{!empty($row->supplier)?$row->supplier:'无' }}  </font></p>
				</td>
            	<td>{{ $row->name }}</td>
            	<td style="color: burlywood;">
            		@if($row->shoptype==0)
            		无标签
            		@elseif($row->shoptype==1)
            		人气商品
            		@elseif($row->shoptype==2)
            		推荐商品
            		@endif
            	</td>
            	...
            	<td>{{$row->cur_periods or ''}}</td>
				<td><BUTTON onclick='alert("正在开发...")'>PC链接</BUTTON><br/><br/><BUTTON onclick='alert("正在开发...")'>H5链接</BUTTON></td>
            	<td>
            		<a href="javascript:void(0);" role="button" data-toggle="modal" class="lookshop" data-id="{{ $row->id }}"><i >查看详情</i></a>
              		<a href="javascript:void(0);" role="button" data-toggle="modal" class="upshop" data-id="{{ $row->id }}"><i >修改</i></a>
            		@if($row->isdeleted==0)
            		 <a href="javascript:void(0);"  role="button" data-toggle="modal" class="delshop" data-id="{{ $row->id }}"><button>下架</button></a>
            		@else
            		<a href="javascript:void(0);" class='xiajia uploadShopAgain' role="button" data-toggle="modal" data-id="{{ $row->id }}"><button>上架</button></a>
            		<a href="javascript:void(0);" class='xiajia' role="button" data-toggle="modal"  data-id="{{ $row->id }}"><button onclick='alert("正在开发...")'>预览</button></a>
            		@endif
            	</td>
            </tr>
        @endforeach
