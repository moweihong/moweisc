      ...
      <tbody>
      	@foreach ($list as $row)
            <tr>
            	...
            	<td>{{ $row->id }}</td>
            	<td>{{ $row->merchants }}</td>
            	<td style="color: burlywood;">{{ date('Y-m-d H:i:s',$row->time) }}</td>
            	<td>
            		
              		<a href="javascript:void(0);" role="button" data-toggle="modal" >
              			<input data-id="{{ $row->id }}" class="delMerchants" type="button" value="删除">
              		</a>
              		<a href="javascript:void(0);" role="button" data-toggle="modal" >
              			<input class="changeMerchants"  data-content="{{ $row->merchants }}" data-id="{{ $row->id }}" type="button" value="编辑">
              		</a>
