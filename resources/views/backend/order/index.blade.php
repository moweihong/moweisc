...
<div class="well">
    <div>
        ...
        <span>供应商：<input value="<?php echo session('order.supplier'); ?>" placeholder="输入供应商..." name="supplier" type="text">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
        <span>订单状态：</span>
      ...
      <tbody>          
      	@foreach ($list as $row)
            <tr>
            	...
            	<td>
            		@if($row->fetchno > 0)
            		{{ date('Y-m-d H:i:s',$row->kaijiang_time/1000) }}
            		@endif
            	</td>
            	<td>
            		@if($row->affirm_time > 0)
            		{{ date('Y-m-d H:i:s',$row->affirm_time) }}
            		@endif
            	</td>
            	<td>
            		@if($row->shiptime > 0)
            		{{ date('Y-m-d H:i:s',$row->shiptime) }}
            		@endif
            	</td>
            	<td>{{ $row->g_name }}</td>
            	<td>{{ $row->supplier }}</td>
            	<td>@if($row->is_virtual==0)  
            			实物 
            		@else 
            			虚拟
            		@endif
