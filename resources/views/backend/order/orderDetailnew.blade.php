        ...
        @else
        	<table class="b-table" border="0" cellpadding="0">
        		...
        		<tr>
                    ...
                    <td bgcolor="#f7f7f7"><span class="order-xd-td">下单：{{date('Y-m-d H:i:s',$order->bid_time/1000) }}</span>
                        <span class="order-xd-td">开奖：
                        	@if($order->kaijiang_time)
                        	{{date('Y-m-d H:i:s',$order->kaijiang_time/1000) }}
                        	@endif
                        </span>
                        <span class="order-xd-td">确址：
                        	@if($order->affirm_time)
                        	{{date('Y-m-d H:i:s',$order->affirm_time) }}
                        	@endif
                        </span>
                        <span class="order-xd-td">发货：
                        	@if($order->shiptime)
                        	{{date('Y-m-d H:i:s',$order->shiptime) }}
                        	@endif
                        </span>
                    </td>
                ...
                <tr>
                    ...
                    <td colspan="2" style="padding: 0px">
                        <div class="order-div-h"><span class="order-h-tit">商品标题</span> <span class="order-h-text">{{$order->g_name}}</span></div>
                        <div class="order-div-h"><span class="order-h-tit">供应商</span> <span class="order-h-text color-f3bd00">{{$order->goods->supplier}}</span></div>
                        <div class="order-div-h"><span class="order-h-tit">价格</span> <span class="order-h-text color-ff0737" style="font-family: microsoft yahei">￥{{$order->goods->money}}</span></div>
                        <div class="order-div-h"><span class="order-h-tit">期数</span> <span class="order-h-text">{{$order->g_periods}}</span></div>
                        <div class="order-div-h"><span class="order-h-tit">类型</span> <span class="order-h-text">@if($order->goods->is_virtua==0)实物@else虚拟@endif</span></div>
                    </td>
                </tr>
                <tr>
                    <td bgcolor="#f7f7f7">下单信息</td>
                    <td colspan="2" style="padding: 0px" bgcolor="#f7f7f7">
                        <div class="order-div-h"><span class="order-h-tit">下单人</span> <span class="order-h-text">{{$order->user->mobile}} </span></div>
                        <div class="order-div-h"><span class="order-h-tit">购买次数</span> <span class="order-h-text order-buy-times"> {{$order->buycount}}</span></div>
                        <div class="order-div-h"><span class="order-h-tit">总金额</span> <span class="order-h-text order-buy-money" style="font-family: microsoft yahei">￥{{$order->buycount*$order->object->minimum}}</span></div>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td colspan="2" style="padding: 0px">
                        ...
                        <div class="order-div-h"><span class="order-h-tit">备注</span> <span class="order-h-text">{{$order->addressjson->notice}}</span></div>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td colspan="2"  style="padding: 0px" bgcolor="#f7f7f7">
                        <div class="order-div-h"><span class="order-h-tit">物流公司 </span> <span class="order-h-text">{{$order->delivery}}</span></div>
                        <div class="order-div-h"><span class="order-h-tit">运单号 </span> <span class="order-h-text order-buy-ydh">{{$order->delivery_code}}</span></div>
                    </td>
                </tr>
