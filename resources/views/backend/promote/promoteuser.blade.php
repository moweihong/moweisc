      ...
      <tbody>
      	@foreach ($userlist as $row)
            <tr>
            	...
            	<td>{{ $row->nousenum }}</td>
            	<td>{{ $row->usenum }}</td>
            	<td>{{ $row->dongjienum }}</td>
            	<td>{{ $row->timeoutnum }}</td>
            	
            	<td>正常</td>
            	<td>
              		<a href="javascript:void(0);" role="button" data-toggle="modal" class="showcardnum" data-uid="{{ $row->usr_id }}"><i >添加卡数</i></a>
            	</td>
            </tr>
        @endforeach
      </tbody>
...
<div id="pageStr" class="pageStr" style="margin: 64px auto;">
	@if(method_exists($userlist,'render'))
    {!! $userlist->render() !!}
    @endif
</div>
