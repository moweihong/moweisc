...
<script>
  ...
  $(".upshop").click(function(){
   ...
   $('#searchNum').click(function(){
  		...
  		$.post('/backend/promote/searchNum',{'startnum':startnum,'endnum':endnum,'_token':"<?php echo e(csrf_token()); ?>"},function(data)
  		{
  			var data=eval('('+data+')');
  			if(data.code>0)
  			{
  ...
  $('#nobindsubmit').click(function(){
  		var startnum=$('#startnum').val();
  		var endnum=$('#endnum').val();
  		$.post('/backend/promote/noBindRecommendid',{'startnum':startnum,'endnum':endnum,'_token':"<?php echo e(csrf_token()); ?>"},function(data)
  		{
  			var data=eval('('+data+')');
  			alert(data.result);	
  		})
   ...
   $('#submitBind').click(function(){
  		...
  		$.post('/backend/promote/submitBind',{'usrphone':usrphone,'upcardnum':upcarnum,'startnum':startnum,'endnum':endnum,'_token':"<?php echo e(csrf_token()); ?>"},function(data)
  		{
  			var data=eval('('+data+')');
  			if(data.code>0)
  			{
