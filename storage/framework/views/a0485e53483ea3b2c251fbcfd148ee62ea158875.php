...
<div class="well">
    <div>
        <form name = 'orderform' method="post" action="<?php echo e(url('backend/order')); ?>" onsubmit="return check()">
        <?php echo csrf_field(); ?>
    ...
    <table class="table">
      ...
      <tbody>          
      	<?php foreach($list as $row): ?>
            <tr>
            	<td rowspan="5">
            		<span>订单号：<?php echo e($row->code); ?></span><br />
            		<span>下单：<?php echo e(date('Y-m-d H:i:s',$row->bid_time/1000)); ?></span><br />
            		<span>开奖：<?php if($row->fetchno > 0): ?><?php echo e(date('Y-m-d H:i:s',$row->kaijiang_time/1000)); ?><?php endif; ?></span><br />
            		<span>确址：<?php if($row->affirm_time > 0): ?><?php echo e(date('Y-m-d H:i:s',$row->affirm_time)); ?><?php endif; ?></span><br />
            		<span>发货：<?php if($row->shiptime > 0): ?><?php echo e(date('Y-m-d H:i:s',$row->shiptime)); ?><?php endif; ?></span><br />
            	</td>
            	
            	
            	<td rowspan="5">
            		<span>商品名称：<?php echo e($row->g_name); ?></span><br />
            		<span>供应商：<?php echo e($row->supplier); ?></span><br />
            		<span>商品价格：<?php echo e($row->money); ?></span>
            		<span>期数：<?php echo e($row->g_periods); ?></span>
            		<span>类型：<?php if($row->is_virtual==0): ?>实物 <?php else: ?>虚拟<?php endif; ?></span>
            	</td>
            	
            	
            	<td rowspan="5">
            		<span>下单人：<?php echo e($row->user->mobile); ?></span><br />
            		<span>购买人次：<?php echo e($row->buycount); ?></span><br />
            	</td>
            	...
            	<td>
                    <?php if($row->fetchno == 0): ?>
                        ...
                        <?php else: ?>
                            否
                        <?php endif; ?>
                    <?php elseif($row->fetchno > 0): ?>
                        中奖号是：<?php echo e($row->fetchno); ?>
                ...
                <td><?php echo e($row->sign); ?><input name="dayin" type="checkbox" value="<?php echo e($row->id); ?>" ></td>
            	<td>

                    <?php if($row->fetchno > 0): ?>
                        <?php if($row->status == 3): ?>
                        <input type="button" class="sendGood" data-id='<?php echo e($row->id); ?>' value="发货">
					    <input type="button" class="ziti" data-id='<?php echo e($row->id); ?>' value="自提">
                        <?php endif; ?>
            		...
            		<a href="<?php echo e(url('backend/order/orderdayin',['id'=>$row->id])); ?>"><button>打印</button></a>
            	</td>
            </tr>
...
<div>
	   合计条数：<?php echo e($num); ?>合计金额：<?php echo e($summoney); ?>
...
<script>
	$('#pldaying').click(function(){
            ...
            $.post('/backend/order/setDayinId',{'_token':"<?php echo e(csrf_token()); ?>",'id':id},
		        function(data){  
	...
	$(".ziti").click(function(){
		...
		layer.confirm('您是否确定自提此订单？', {
		  btn: ['确定','取消'] //按钮
			...
			$.post("<?php echo e(url('/backend/order/takeorder')); ?>",{'id':id,'_token':"<?php echo e(csrf_token()); ?>"},function(data){
				if(data==null){
					layer.msg('服务端错误');
				}        
				if (data.status==1){
	...
	$("#plziti").click(function(){
		...
		layer.confirm('您是否确定自提这些订单，请认真对清楚？', {
		  btn: ['确定','取消'] //按钮
			...
			$.post("<?php echo e(url('/backend/order/takeorderpl')); ?>",{'id':id,'_token':"<?php echo e(csrf_token()); ?>"},function(data){
				if(data==null){
					layer.msg('服务端错误');
				}        
				if (data.status==1){
