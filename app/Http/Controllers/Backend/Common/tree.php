	...
	*/
	...
	function order ($array,$pid=0)
	{
	    ...
	    foreach($array as $v){
	        if($v['parentid']==$pid){
	            $arr[] = $v;
	            $arr = array_merge($arr,$this->order($array,$v['parentid']));
	        }
	    }
	    return $arr;
    }
	
	public function getTree(&$list,$pid=0,$level=0,$html='--')
	{
	    static $tree = array();
	    foreach($list as $v){
	        if($v['parentid'] == $pid){
	            ...
	            $tree[] = $v;
	            $this->getTree($list,$v['cateid'],$level+1);
	        } 
	    }
	    return $tree;
	...
	//百度编辑器过滤
	public function editor_safe_replace($content)
	{
	    $tags = array(
	        "'<iframe[^>]*?>.*?</iframe>'is",
	        "'<frame[^>]*?>.*?</frame>'is",
