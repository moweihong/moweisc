...
m3 modify by tangzhe 2016-08-10 推荐人封装
*/
...
class AuthController extends ForeController
{
    ...
    public function register(Request $request)
    {
        ...
        if($mobile != session('verifyCode_mobile')){
			if($_SERVER['HTTP_HOST']!='ykg.tz' && $_SERVER['HTTP_HOST']!='1kg.muyouguanxi.com')
...
//            return response()->json(array('status' => -6, 'msg' => '您的手机号码存在异常，请您更换手机号码注册，或联系客服核实！'));
//        }
        //end
        ...
        $data['user_name']  = $this->getParam('mobile');
        $data['user_pass']  = md5($this->getParam('password'));
        //$data['user_email'] = 'yikuaigou@ccfax.cn';
        $data['user_phone'] = $this->getParam('mobile');

		$data['platform_source'] = 3;  //平台来源一块购
		$data['equipment_source'] = 1;  //设备来源 1pc、2ios、3、安卓
			
		//$data['recommend_id'] = $recommend_id; //推荐人ID
		//$data['user_type'] = 1;
        //$data['user_regtype'] = 1;
        //$data['pin_pass'] = '123456';
        ...
        if ($result['code'] > 0) {
           ...
           // $member->kl_bean = 100;  //注册赠送100块乐豆 16.6.14 不送快乐豆 by byl
            $member->mobile = $this->getParam('mobile');
            $member->user_name = $this->getParam('mobile');
...
//                $log_reg->save();
                ...
                //推荐人增加块乐豆
//                 if($recommend_id){
//                     $recommender = $member->where('usr_id', $recommend_id)->first();
//                     $recommender->kl_bean += $this->reg_add_bean;
//                     $recommender->save();                   
//                     //写块乐豆log
//                     $log = new Userpoint();
//                     $log->usr_id = $recommend_id;
//                     $log->type = 3;
//                     $log->pay = '邀请好友';
...
//                 }
	            ...
	            //注册登陆成功
	            DB::table('tab_login_log')->insert(['usr_id' => $result['code'], 'is_reg' => 1, 'platform' => 0, 'time' => time()]);   //插入登陆记录
...
//			    $log_reg->save();
            ...
            //登陆成功(增加连续登陆字段：mowei)
            $sql="SELECT num FROM `tab_login_log` WHERE  FROM_UNIXTIME( TIME, '%Y%m%d' )= DATE_SUB(CURDATE(),INTERVAL 1 DAY) and usr_id=".$user_info['usr_id'].' limit 1';
			$result=DB::select($sql);
			if(empty($result)){$num=1;}else{$num=$result[0]->num+1;}
	        DB::table('tab_login_log')->insert(['usr_id' => $user_info['usr_id'], 'is_reg' => 0, 'platform' => 0,  'time' => time(),  'num' => $num]);   //插入登陆记录
...
//         }
    ...
    public function synchronize(){
        ...
        if (!empty($session_id)) {
            $session_login_time = session('user.login_time');
            //查询用户最近的登陆时间
            $login_time = UserRepositoryFacade::findLoginTime($session_id);
            if ($login_time != $session_login_time) {
                session()->flush();
                echo json_encode(array('status'=>-10001, 'message'=>'logout'));
            }else{
                echo json_encode(array('status'=>0, 'message'=>'islogin'));
            }
        }
    }
    ...
    //微信登陆回调  by  baoyaoling
    public function wxCallBack()
    {
        $code = $this->getParam('code');
        $state = $this->getParam('state');
        if(session('login_state') == $state && $state != NULL){
            //session()->forget('login_state');//只用一次
            $url = config('global.weixin.access_token').'?appid='.config('global.weixin.AppID').'&secret='.config('global.weixin.AppSecret').'&code='.$code.'&grant_type=authorization_code';
            ...
            if($res){
                ...
                //不存在就绑定手机
                $getUserinfoUrl = 'https://api.weixin.qq.com/sns/userinfo?access_token=' . $result['access_token'] . '&openid=' . $openid;
                $userInfo = json_decode($this->httpGet($getUserinfoUrl),true);
                if(!empty($userInfo['errcode']) && $userInfo['errcode'] == 40003){
                    echo $userInfo['errmsg'];exit;
                }else{
                    $headimgurl = $userInfo['headimgurl'];
                    $nickname = $userInfo['nickname'];
                    return view('foreground.binduser2',['headimgurl'=>$headimgurl,'statueCode'=>session('login_state'),'invite_code'=>session('invite_code'),'nickname'=>$nickname]);
                }
            }
        }else{
            return redirect('/login');
    ...
    public function quickLogin($user_id,$mobile,$openid,$unionid=NULL)
    {
        ...
        if($unionid == NULL){
         ...
         $return['status'] = 0;
         $return['msg'] = 'success';
         $return['refer'] = !empty(session('url_refer')) ? session('url_refer'):'/freeday';
         session()->forget('url_refer');
         return $return;
    }
...
//            $member->kl_bean = 100;  //注册赠送100块乐豆
            ...
            if($member->save()){
                ...
                $return['status'] = 0;
                $return['msg'] = '注册成功';
	            $return['refer'] = !empty(session('url_refer')) ? session('url_refer'):'/index';
            }
        } else {
            $return['status'] = -1;
            $return['msg'] =  $result['resultText'];
    ...
    /*
     * 用户中心 安全中心 绑定微信回调接口  16.6.14 by byl
     */
    public function wxCenterCallBack()
    {
        $code = $this->getParam('code');
        $state = $this->getParam('state');
        if(session('wx_state') == $state && $state != NULL){
...
//            session(['wx_unionid' => $unionid]);
            ...
            if($res){
                //存在就提示错误               
                return redirect('/user/security')->with('wx_error', '您的微信号已被绑定！');
            }else{
                //不存在就绑定
                $uid = session('user.id');
                Member::where('usr_id','=',$uid)->update(['wx_pc_openid'=>$openid,'wx_unionid'=>$unionid]);
                return redirect('/user/security')->with('wx_error', '绑定成功！');
            }
        ...
        if(session('login_state') == $state && $state != NULL){
            ...
            if($res){
                ...
                if($userInfo['ret'] == 0){
                     ...
                     $nickname = $userInfo['nickname'];
                     return view('foreground.binduser2',['headimgurl'=>$headimgurl,'nickname'=>$nickname,'statueCode'=>session('login_state'),'invite_code'=>session('invite_code')]);
                }else{
                     echo("The state does not match. You may be a victim of CSRF.");
                }
    ...
    /*
     * 用户中心 安全中心 绑定qq回调接口  16.6.16 by byl
     */
    public function qqCenterCallBack()
    {
        $code = $this->getParam('code');
        $state = $this->getParam('state');
        if(session('wx_state') == $state && $state != NULL){
            ...
            if($res){
                //存在就提示错误               
                return redirect('/user/security')->with('wx_error', '您的QQ号已被绑定！');
            }else{
                //不存在就绑定
                $uid = session('user.id');
                Member::where('usr_id','=',$uid)->update(['qq_openid'=>$openid]);
                return redirect('/user/security')->with('wx_error', '绑定成功！');
            }
