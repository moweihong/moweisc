...
class ProductsController extends ForeController
{
    ...
    //根据商品o_id查出所有购买记录
    public function getAllBuyRecord($o_id)
    {
        $data = UserRepositoryFacade::findBuyRecordWithId($o_id);
        return $this->packageData($data);
    }
    ...
    //根据商品o_id查询商品是否处于开奖状态
    public  function getGoodsStatus($o_id){

        $data['status']=ProductRepositoryFacade::findProductStatus($o_id);
        return $this->packageData($data);
