	...
	$.fn.jcImgScroll = function(options) {
		...
		return this.each(function() {
			var _SELF = $(this),
				...
				object = function(length){
					...
					this.arrow = function(l,r,w,h,x,y){
						//style=\"position:absolute;cursor:pointer;z-index:10;width:"+w+"px;height:"+h+"px;left:"+(l-w-x)+"px;\"
						//style=\"position:absolute;cursor:pointer;z-index:10;width:"+w+"px;height:"+h+"px;left:"+(r+x)+"px;top:"+y+"px;\"
						return "<em class=\"sPrev\" id=\"sPrev\" ></em><em class=\"sNext\" id=\"sNext\" ></em>";
					};
			...
			object.prototype.Initial = function(callback){
				...
				this.arrImgObj = [];
				this.arrData = [];
				this.centerIdx =options.selectedImg;// Math.round(this.length/2)-1;
			...
			var $prev = $("#sPrev",_SELF),
				...
				$dd = $("dd",_SELF);
			$prev.click(function(){
				/*if($(".img_shadow").eq(1).css("left")=="0px"){
					$(".img_shadow").eq(0).before($(".img_shadow").eq(9));
				}else{
					if($(".img_shadow").eq(0).css("left")=="0px"){
						return false;
					}
					$(".img_shadow_left").show();
					$(".img_shadow_right").show();
				}*/
				_IDX = _IDX -1;
				if(_IDX > 0) {
			...
			$next.click(function(){
				/*if($(".img_shadow").eq($(".img_shadow").length-2).css("left")=="0px"){
					$(".img_shadow_right").hide();
				}else{
					if($(".img_shadow").eq($(".img_shadow").length-1).css("left")=="0px"){
						return false;
					}
					$(".img_shadow_right").show();
					$(".img_shadow_left").show();
