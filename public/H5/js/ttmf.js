...
$(".invite_friend_con").click(function () {
	...
	$('.join_percentage').text(total_invite + '/' + total_need);
...
$("#hand_shank,#join_btn_normal").click(function () {
    //  $(".number_container ul li").each(function(n){
    if(kl_flag == -1){
    	//login_alert();
    	window.location.href = '/login_m';
    	return false;
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status == 0) {
                    ...
                    $('.not_enough_bean').text(res.data.kl_bean);
...
var tmp=setInterval(function(){
    $(".good_news_center>ul li").eq(index).slideLeftHide("4000");
    index++;
